...
with st.container():
    ...
    with col1:
        st.text_input(label="角色名", key="bot_name", value="黛玉", on_change=lambda : st.session_state["meta"].update(bot_name=st.session_state["bot_name"]), help="模型所扮演的角色的名字，不可以为空")
        st.text_area(label="角色人设", key="bot_info", value="容颜似蹙非蹙，含情目，病态美，性格敏感细腻，聪慧过人，出身名门，却身世可怜。特征是两弯罥烟眉，闲静如姣花照水，行动处似弱柳扶风，心较比干多一窍，病如西子胜三分。", on_change=lambda : st.session_state["meta"].update(bot_info=st.session_state["bot_info"]), help="角色的详细人设信息，不可以为空")

    with col2:
        st.text_input(label="用户名", value="宝玉", key="user_name", on_change=lambda : st.session_state["meta"].update(user_name=st.session_state["user_name"]), help="用户的名字，默认为用户")
        st.text_area(label="用户人设", value="外表英俊，中秋之月面容，春晓之花色泽，性格叛逆又多情，不拘世俗，生于富贵之家，身佩美玉，具有天然风骚与万种情思。特征是束发嵌宝紫金冠，二龙抢珠金抹额，情感丰富，似傻如狂", key="user_info", on_change=lambda : st.session_state["meta"].update(user_info=st.session_state["user_info"]), help="用户的详细人设信息，可以为空")
